---
title: Introduction
description: "Give your AI agents memory they control"
---

<img
  className="block dark:hidden"
  src="/images/hero-light.svg"
  alt="Hero Light"
/>
<img
  className="hidden dark:block"
  src="/images/hero-dark.svg"
  alt="Hero Dark"
/>

# LeedAB

**Give your AI agents perfect memory.** LeedAB is a simple tool that lets your agents remember important information, learn from interactions, and maintain context across conversations.

## The Problem

AI agents forget everything between conversations. They can't learn from experience, track user preferences, or maintain context. You're stuck managing memory manually.

## The Solution

Add memory to your agent in 3 lines:

```python
# Add memory to any agent
memory = Memory(api_key="YOUR_API_KEY")
agent = YourAgent(
    tools=[memory.remember, memory.recall]
)

# Agent remembers important context
user: "I prefer morning meetings and hate video calls"
agent: memory.remember("User prefers morning meetings, dislikes video calls")

# Later, agent uses that knowledge
user: "Schedule our sync"
agent: memory.recall("meeting preferences")
agent: "I'll schedule a morning audio call for you"
```

## Real Impact

<CardGroup cols={2}>
  <Card
    title="Faster Support"
    icon="clock"
  >
    Agents remember every customer interaction, past issues, and resolutions
  </Card>
  <Card
    title="Better Development"
    icon="rocket"
  >
    Code assistants remember your entire codebase context and patterns
  </Card>
  <Card
    title="No Repetition"
    icon="arrows-rotate"
  >
    Users never repeat themselves - agents remember everything relevant
  </Card>
  <Card
    title="Self-Improving Agents"
    icon="chart-line"
  >
    Agents learn from every interaction and get smarter over time
  </Card>
</CardGroup>

## 5-Minute Integration

```python
from agentmind import Memory

# 1. Initialize memory
memory = Memory(api_key="YOUR_API_KEY")

# 2. Give your agent memory tools
def remember_tool(content: str):
    """Agent decides what to store"""
    return memory.remember(content)

def recall_tool(query: str):
    """Agent searches its memory"""
    return memory.recall(query)

# 3. Let your agent handle the rest
agent = YourAgent(tools=[remember_tool, recall_tool])
```

## Deploy Your Agent as an API

Already have your agent configured? Deploy it instantly with one API call:

```python
import requests

# Chat with your agent via API
response = requests.post(
    "https://api.leedab.com/api/v1/agent/chat",
    headers={"Authorization": "Bearer am_live_your_key"},
    json={
        "message": "What do you know about my project?",
        "user_id": "customer_123",  # Segment memories per end-user
        "stream": False
    }
)

print(response.json()["response"])
```

Your agent has access to all your memories, custom tools, and web search - ready for production.

## Who's Already Using It

**Customer Support** → *"Our agents resolve tickets faster by remembering every customer's history"*

**Dev Tools** → *"Code assistants that remember your entire project context and coding style"*

**Sales Teams** → *"Every interaction remembered - better qualified leads and follow-ups"*

**Healthcare** → *"Complete patient history at agents' fingertips for better care"*

## Start Now

<CardGroup cols={2}>
  <Card
    title="5-Minute Quickstart"
    icon="bolt"
    href="/quickstart"
  >
    Integration so simple, you'll be done before your coffee cools
  </Card>
  <Card
    title="See It In Action"
    icon="play"
    href="/essentials/quick-examples"
  >
    Real agent examples you can run right now
  </Card>
</CardGroup>
